--[[
    High-Performance ESP Library v1.0 - Fixed Ray Issue
...
local function GetLocalAttachment()
    ...
    -- 修复：如果 Attachment 丢失、失效或不在当前最新的 Root 下，则重新创建
    if not localAttachment or not localAttachment.Parent or localAttachment.Parent ~= root then
        if localAttachment then 
            pcall(function() localAttachment:Destroy() end) 
        end
        localAttachment = Instance.new("Attachment")
        localAttachment.Name = "ESP_LocalAttachment"
        localAttachment.Parent = root
        
        -- 核心修复：当本地起点更新时，立即同步所有活跃射线的 Attachment0
        for _, data in pairs(ESP.activePlayers) do
            if data.beam then data.beam.Attachment0 = localAttachment end
        end
...
function ESP:UpdatePlayerVisuals(player)
    ...
    if data.beam.Enabled then
        ...
        -- 修复：实时验证 Attachment0，防止断裂
        local currentAtt0 = GetLocalAttachment()
        if currentAtt0 and data.beam.Attachment0 ~= currentAtt0 then
            data.beam.Attachment0 = currentAtt0
        end
    end
end
...
function ESP:CheckCondition(instance)
    ...
    if data and data.conditionFunc then
        local success, passed = pcall(function() return data.conditionFunc(instance) end)
        if not success or passed == false then
            self:RemoveObject(instance)
            return false
        end
    end
...
LocalPlayer.CharacterAdded:Connect(function(char)
    localAttachment = nil 
    task.wait(0.5) 
    GetLocalAttachment() -- 强制重置起点并刷新所有射线连接
end)
